        ...
        <div className="relative flex items-center">
          ...
          {(endIcon || error) && (
            <div className="absolute right-3 text-slate-400 flex items-center pointer-events-none">
              {error ? <AlertCircle className="w-4 h-4 text-rose-500" /> : endIcon}
            </div>
          )}
        </div>

        {error ? (
          <p id={errorId} className="text-[11px] text-rose-600 dark:text-rose-400 font-medium flex items-center gap-1">
            <span>{error}</span>
          </p>
        ) : helperText ? (
